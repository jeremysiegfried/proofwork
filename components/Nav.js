          ...
          <div className="flex flex-col gap-3">
            ...
            {loading ? null : user ? (
              <div className="flex items-center justify-between">
                ...
                <button onClick={function() { signOut(); closeMobile() }} className="text-xs text-pw-muted hover:text-pw-text2">
                  Sign out
                </button>
              </div>
            ) : (
              <div className="flex gap-2">
                <Link href="/login" className="flex-1 py-2.5 rounded-lg border border-pw-border text-pw-text1 font-bold text-sm text-center" onClick={closeMobile}>
                  Log in
                </Link>
                <Link href="/signup" className="flex-1 py-2.5 rounded-lg bg-pw-green text-white font-bold text-sm text-center" onClick={closeMobile}>
                  Sign up
                </Link>
              </div>
            )}
